...
XSIPLUGINCALLBACK CStatus MomentumConsHinge_Define( CRef& in_ctxt )
{
   ...
   oPDef = oFactory.CreateParamDef(L"angularMin",CValue::siFloat,siAnimatable | siPersistable,L"angularMin",L"angularMin",180,-100000,100000,-180,180);
   oCustomOperator.AddParameter(oPDef,oParam);

   oPDef = oFactory.CreateParamDef(L"angularMax",CValue::siFloat,siAnimatable | siPersistable,L"angularMax",L"angularMax",-180,-100000,100000,-180,180);
   oCustomOperator.AddParameter(oPDef,oParam);
   ...
   oPDef = oFactory.CreateParamDef(L"angularMotorForce",CValue::siFloat,siAnimatable | siPersistable,L"angularMotorForce",L"angularMotorForce",25,-10000,10000,-100,100);
   oCustomOperator.AddParameter(oPDef,oParam);

   oPDef = oFactory.CreateParamDef(L"angularMotorVelocity",CValue::siFloat,siAnimatable | siPersistable,L"angularMotorVelocity",L"angularMotorVelocity",2,0,10000,0,10);
...
XSIPLUGINCALLBACK CStatus MomentumConsHinge_DefineLayout( CRef& in_ctxt )
{
...
XSIPLUGINCALLBACK CStatus MomentumConsHinge_Init( CRef& in_ctxt )
{
   gInstanceCount++;
   Context ctxt( in_ctxt );
...
XSIPLUGINCALLBACK CStatus MomentumConsHinge_Term( CRef& in_ctxt )
{
   gInstanceCount--;
   Context ctxt( in_ctxt );
...
XSIPLUGINCALLBACK CStatus MomentumConsHinge_Update( CRef& in_ctxt )
{
   ...
   if(consRef == NULL)
   {
      ...
      // let's get two axis
      btVector3 axisA = consRef->pivotA.getBasis() * btVector3(1,0,0);
      btVector3 axisB = consRef->pivotB.getBasis() * btVector3(1,0,0);

      consRef->constraint = new btHingeConstraint(
            *consRef->bodyA,*consRef->bodyB,consRef->pivotA.getOrigin(),consRef->pivotB.getOrigin(),axisA,axisB,true);
   ...
   if(consRef != NULL)
   {
      ...
      btHingeConstraint * hinge = (btHingeConstraint *)consRef->constraint;

      hinge->setLimit(DegreesToRadians((float)ctxt.GetParameterValue(L"angularMin")),DegreesToRadians((float)ctxt.GetParameterValue(L"angularMax")));
      hinge->enableAngularMotor((bool)ctxt.GetParameterValue(L"angularMotor"),
                                (float)ctxt.GetParameterValue(L"angularMotorVelocity"),
                                (float)ctxt.GetParameterValue(L"angularMotorForce"));
   }
...
XSIPLUGINCALLBACK CStatus apply_MomentumConsHinge_Init( CRef& in_ctxt )

{
   ...
   oCmd.PutDescription(L"Create an instance of MomentumConsHinge operator");
...
XSIPLUGINCALLBACK CStatus apply_MomentumConsHinge_Execute( CRef& in_ctxt )

{
   ...
   Null consNull1;
   obj1.AddNull(L"Hinge_pivot",consNull1);
   consNull1.GetKinematics().GetGlobal().PutTransform(xfA);
   consNull1.GetActivePrimitive().PutParameterValue(L"primary_icon", (LONG)0);
   consNull1.GetActivePrimitive().PutParameterValue(L"shadow_icon", (LONG)2);
   consNull1.GetActivePrimitive().PutParameterValue(L"shadow_scaleX", 0.1f);
   consNull1.GetActivePrimitive().PutParameterValue(L"shadow_scaleY", 2.0f);
   consNull1.GetActivePrimitive().PutParameterValue(L"shadow_scaleZ", 2.0f);

   // create the operator

   CustomOperator newOp = Application().GetFactory().CreateObject(L"MomentumConsHinge");
