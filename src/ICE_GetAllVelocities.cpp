...
CStatus Register_MOM_GetAllVelocities( PluginRegistrar& in_reg )
{
	ICENodeDef nodeDef;
	nodeDef = Application().GetFactory().CreateICENodeDef(L"MOM_GetAllVelocities",L"MOM_GetAllVelocities");
	...
	// Add output ports.
	st = nodeDef.AddOutputPort(ID_OUT_Velocities,siICENodeDataMatrix33,siICENodeStructureArray,siICENodeContextSingleton,L"Velocities",L"Velocities",ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
	st.AssertSucceeded( ) ;
...
SICALLBACK MOM_GetAllVelocities_Evaluate( ICENodeContext& in_ctxt )
{
	...
	switch( out_portID )
	{
		case ID_OUT_Velocities:
		{
         ...
         gSimulation->GetBodyVelocities(&pos,&vel,(int)modeData[0]);
