...
CStatus Register_MOM_GetRigidBodyXF( PluginRegistrar& in_reg )
{
	ICENodeDef nodeDef;
	nodeDef = Application().GetFactory().CreateICENodeDef(L"MOM_GetRigidBodyXF",L"MOM_GetRigidBodyXF");
	...
	// Add output ports.
	st = nodeDef.AddOutputPort(ID_OUT_xfs,siICENodeDataMatrix33,siICENodeStructureSingle,siICENodeContextComponent0D,L"xfs",L"xfs",ID_UNDEF,ID_UNDEF,ID_UNDEF);
	st.AssertSucceeded( ) ;
...
SICALLBACK MOM_GetRigidBodyXF_Evaluate( ICENodeContext& in_ctxt )
{
	...
	switch( out_portID )
	{
		case ID_OUT_xfs :
		{
         ...
         // get the index set iterator
			...
			{
            rbd_ID.primary = (int)baseData[it];
            ...
            if(bodyRef != NULL)
            {
               ...
               rot.SetFromQuaternion(CQuaternion(rbdRot.getW(),rbdRot.getX(),rbdRot.getY(),rbdRot.getZ()));
               angles = rot.GetXYZAngles();

               outData[it].Set(rbdPos.getX(),rbdPos.getY(),rbdPos.getZ(),
                               RadiansToDegrees(angles.GetX()),RadiansToDegrees(angles.GetY()),RadiansToDegrees(angles.GetZ()),
                               0,0,0);
            }
			}
		}
